                ...
                <table class="table is-fullwidth is-hoverable is-striped">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                @php $count = $crew->users->count(); @endphp
                                @if ($count === 0)
                                    {{ $crew->capacity }}
                                @else
                                    <a href="#" onclick="showUsers(event, {{ $crew->id }})">{{ $count . '/' . $crew->capacity }}</a>
                                    <div id="crew-users-{{ $crew->id }}" style="display: none;">
                                        <ul style="list-style-type: none; padding: 0;">
                                            @foreach ($crew->users as $user)
                                                <li style="margin-bottom: 10px; display: flex; justify-content: space-between; align-items: center;">
                                                    ...
                                                    <div>
                                                        <button class="button is-small is-danger" onclick="removeUser(event, {{ $crew->id }}, {{ $user->id }})">Eliminar</button>
                                                        <a href="{{ route('back.users.show', $user->id) }}" class="button is-small is-success">Ver</a>
                                                    </div>
                                                </li>
                                            @endforeach
                                        </ul>
                            ...
                            <td class="foundation">{{ $crew->foundation }}</td>
                            <td>
                                ...
                                <div style="display: flex; align-items: center; gap: 8px; flex-wrap: nowrap;">
                                    ...
                                    <form action="{{ route('back.crews.destroy', $crew->id) }}" method="POST" style="display: inline;">
                                        ...
                                        <button
                                            type="submit"
                                            onclick="deleteCrew(event, {{ $crew->id }})"
